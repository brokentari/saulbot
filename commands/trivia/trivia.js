...
class pokeTrivia extends commando.Command {
  ...
  async run(message, args) {
    ...
    function padNum(number) {
      // adds 00 to conform with the image folder/names
    ...
    if (args.includes("score")) {
      if (args.includes("@")) {
        ...
        message.channel.send(
          args.substring(6, args.length) + "'s score is " + PointSystem.get(id)
        );
      } else {
        //otherwise, the author's score will be referenced
        message.channel.send(
          message.author.username +
            "'s score is " +
            PointSystem.get(message.author.id) +
            " points"
        );
      }
    } else {
